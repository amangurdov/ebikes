...
import makeInOutStock from '@salesforce/apex/ProductController.makeInOutStock';
...
export default class ProductCard extends NavigationMixin(LightningElement) {
    ...
    activeSections = ['electiccomponent', 'frame', 'brakes', 'rear'];
    connectedCallback() {
    ...
    handleRecordLoaded(event) {
        ...
        makeInOutStock({productName : this.productName})
            .then(result => {
                var resultObj = JSON.parse(result);
                if (parseInt(resultObj.currentInventory) > 0) {
                    this.productName += ' - In Stock';
                } else {
                    this.productName += ' - Out of Stock';
                }
            })
            .catch(error => {
                console.log('handleRecordLoaded ',error);
            });
    }
